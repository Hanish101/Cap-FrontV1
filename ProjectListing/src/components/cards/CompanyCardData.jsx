...
export default function CompanyCardData({ companyData, handleBusDetails }) {
    ...
    return (
        ...
        <div className="w-96 h-72 border-4 border-lightgray rounded-lg shadow-md overflow-hidden" style={{ borderColor:randomColor }} onClick={handleDetailClick}>
            <div className="px-4 py-2 text-sm font-bold" style={{ backgroundColor: randomColor}}>{formattedDate}</div>
            <div className="flex items-center px-4 py-2">
                ...
                <div className="flex flex-col">
                    ...
                    <div className="text-sm text-gray-600">
                        {companyData.rating && companyData.rating.length > 0 && (
                            <div className="flex items-center">
                                <Star className="text-yellow-400 mr-1" />
                                <span className="text-black-400">
                                    {companyData.rating.reduce((sum, rating) => sum + rating, 0) / companyData.rating.length}
                                </span>
                            </div>
                        )}
                    </div>
