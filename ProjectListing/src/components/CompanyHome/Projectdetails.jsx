...
export default function Projectdetails({ projectData, projectView }) {
  ...
  useEffect(() => {
    ...
    const fetchTeamArray = async () => {
      const promises = projectData.team.map(fetchData);
      const fetchedData = await Promise.all(promises);
  ...
  return (
    <div className='w-full flex flex-col bg-navbar'>
      <ToastContainer />
      <div className="h-300 mx-4 bg-white">
        <div className="p-4 rounded-t-xl">
          <div className="flex items-center justify-between mb-4">
            <div className="text-2xl md:text-4xl text-center font-bold mb-2 text-black">{projectData.project_name}</div>
            {projectView === 'company' ? (
              <div>
          ...
          <p className="text-base md:text-lg mb-2">Description: {projectData.description}</p>
          <p className="text-base md:text-lg mb-2">Price: ${projectData.price}/hr</p>
          <div className="flex items-center mb-4">
            ...
            <div className="flex items-center">
              ...
              <span className="text-black">{projectData.team.length} Team members</span>
            </div>
          </div>
          <div className="flex flex-wrap">
            {projectData.technology.map((tech, index) => (
              <div
                key={index}
                className="text-black py-1 px-3 rounded-lg text-sm md:text-base mr-2 mb-2"
              >
                {tech}
              </div>
            ))}
        ...
        <div className="flex justify-end px-4 py-2 rounded-b-lg">
          <div className="text-black font-bold text-md text-end">{dateFormat(projectData.createdAt)}</div>
        </div>
      </div>


      <div className='mx-6 text-lg font-semibold'>Developers </div>
      <div className='flex-1 grid grid-cols-5 gap-4 justify-end rounded-xl mx-4 p-2 pt-10'>
      ...
      <div className='mx-6 text-lg font-semibold'>Team members </div>
      <div className='flex-1 grid grid-cols-5 gap-4 justify-end rounded-xl mx-4 p-2 pt-10'>
        {teams.map((developer) => (
          // Check for missing data in developer object
          developer.data && developer.data.id ? (
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex w-full items-center justify-between">
                  <div className="mt-3 w-full text-center sm:mt-0 sm:text-left">
                            ...
                            <div className="flex flex-wrap">
                              {developerData.skills && developerData.skills.length > 0 && (
                                developerData.skills.map((skill, index) => (
                                  <button key={index} className="bg-gray-200 py-1 px-3 rounded-lg text-md mr-2 mb-2">
                                    {skill}
                                  </button>
                                ))
                              )}
                    ...
                    <div>
                      <div className='flex'>
                        <h1 class>Rate and provide a testimonial:</h1>
                        <h1 class>Rating:</h1>
                        <input
                          type="number"
                          value={rating}
                          min="1"
                          max="5"
                          className="w-10 mx-4 px-2 py-1 rounded border-2 border-gray-500 focus:border-secondary focus:outline-none"
